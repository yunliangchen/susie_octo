...
def main(_):
    ...
    def get_action(obs, goal_obs, rng):
        # create `task` dict
        task = model.create_tasks(goals={"image_primary": goal_obs[None]})  # for goal-conditioned
        
        if len(obs.shape) == 4:
            input_images = obs[None]
        observation = {
        ...
        norm_actions = model.sample_actions(observation, task, rng=rng)
